#!/usr/bin/env python3
#v20_mpv - Replaced omxd with mpv player
#Changes - Migrated from omxd to mpv with JSON IPC for playlist management
...
class MPVController:
    ...
    def start_mpv_daemon(self):
        ...
        try:
            ...
            logging.error("Failed to start mpv: %s" % str(e))
            return False
    ...
    def send_command(self, command):
        ...
        try:
            ...
            logging.error("MPV command failed: %s" % str(e))
            return None
...
# Global MPV controller instance
mpv = MPVController()
...
def filechecker(x, y, z):
    ...
    if not os.path.isfile(z):
        logger.warning("Video file not found: %s" % z)
    ...
    try:
        ...
        logger.error("filechecker error: %s" % str(e))
...
try:
     ...
     serialport.close()
     print(version)
     logger.info("pitron_v20_mpv - SAM COMM Version %s" % version)
