...
hdlr = logging.FileHandler('./Tron_states_tls.log')
...
serialport = serial.Serial("COM3", 57600, timeout=0.5)
 ...
 #   if not states:
  #      if not flags['recog_start']:
   ...
   # else:
    for state in states:
            state = state.rstrip()
            if b'attract_mode_active' in state:
                if not flags['attract_mode_active']:
                    logger.info("Attract is running")
                    #run attract stern video
                    #changed output to run vlc instead of omx
                    #result = subprocess.check_output(['"C:\Program Files (x86)\VideoLAN\VLC\vlc.exe"'])
                        #attract.mp4 -f -L --no-overlay --no-video-title-show'
                    ##logger.debug(result)
