...
print version
if version > 26:
     print "version verfied"
     result = filechecker(False, 'I', '/home/pi/media/system/rom_verified.mp4')
else:
     print "firmware rom needs to be update"
     result = filechecker(False, 'I', '/home/pi/media/system/error_coms.mp4')
    ...
    try:
        ...
        while 1:
            ...
            #Default state if bank is up
                        print("Game Start- Entering the Grid")
                        if not flags['null_mode_active']:   
                          ...
                          #            logger.info("Player 1")
                                      #run attract stern video
                          #            result = filechecker(True, 'I', '/home/pi/media/attract/player_1.mp4')
                          #            time.sleep(5)
                          #            logger.debug(result)
            ...
            #Needs to be default state if bank is not down 
                        ...
                        if not flags['game_over']:
                            result = subprocess.check_output(['omxd', 'X'])
                            logger.debug(result)
                            result = subprocess.check_output(['omxd', 'I', '/home/pi/media/attract/game-over.mp4'])
                            logger.debug(result)
                        ...
                        if not flags['disc_mb_active']:
                         #   result = subprocess.check_output(['omxd', 'X'])
                            logger.debug(result)
                            result = subprocess.check_output(['omxd', 'I', '/home/pi/media/discmb/discmb_active.mp4'])
                            logger.debug(result)
        ...
        result = filechecker(False, 'I', '/home/pi/media/system/serial_failure.mp4')
